...
public class VideoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new VideoActivity.LongOperation().execute();

        rv_img_download.setLayoutManager(new GridLayoutManager(VideoActivity.this, 3));
        rv_img_download.setNestedScrollingEnabled(false);
        allDocumentAdapter = new AllDocumentAdapter(imgMainDownloadList, VideoActivity.this, "video");
        rv_img_download.setAdapter(allDocumentAdapter);

        rv_img_download_list.setLayoutManager(new LinearLayoutManager(VideoActivity.this, RecyclerView.VERTICAL, false));
        rv_img_download_list.setNestedScrollingEnabled(false);
        allDocListAdapter = new AllDocListAdapter(imgMainDownloadList, VideoActivity.this, "video");
        rv_img_download_list.setAdapter(allDocListAdapter);

        img_grid_list.setOnClickListener(v -> {
            if (rl_fram.getVisibility() == View.VISIBLE) {
                img_grid_list.setImageDrawable(VideoActivity.this.getResources().getDrawable(R.drawable.list_view));
                rl_List.setVisibility(View.VISIBLE);
                rl_fram.setVisibility(View.GONE);
            } else {
                img_grid_list.setImageDrawable(VideoActivity.this.getResources().getDrawable(R.drawable.gird_view));
...
//                        Log.e("LLLLL_Filename_img: ", imagePath.getParentFile().getName());
    ...
    private final class LongOperation extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            load_image_files(new File(DownloadData.docDirNamedir));
            return "Execute";
        }
