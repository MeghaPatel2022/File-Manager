...
internal class UwMediaPickerActivity : AppCompatActivity(), CoroutineScope {
        ...
        launch {
            try {
                ...
                recyclerView.adapter =
                    GalleryMediaRvAdapter(mediaBucketsList, galleryMediaBucketClickListener)
            } catch (e: Exception) {
                Snackbar
                    .make(
                        tvToolbarDone,
    ...
    private fun setupMediaBucketRecyclerView() {
        ...
        recyclerView.adapter = GalleryMediaRvAdapter(
            mediaList,
            galleryMediaClickListener,
            object : GalleryMediaOnLongClickListener {
            ...
            launch {
                try {
                    taskOpenMediaBucket = async(Dispatchers.IO) {
                        ...
                        when (settings.galleryMode) {
                            UwMediaPicker.GalleryMode.ImageGallery -> {
                                galleryMediaProvider.getImagesOfBucket(
                                    mediaBucketsList[position].id!!,
                                    selectedMediaPaths
                                )
                            }
                            UwMediaPicker.GalleryMode.VideoGallery -> {
                                galleryMediaProvider.getVideosOfBucket(
                                    mediaBucketsList[position].id!!,
                                    selectedMediaPaths
                                )
                            }
                            UwMediaPicker.GalleryMode.ImageAndVideoGallery -> {
                                galleryMediaProvider.getImagesAndVideosOfBucket(
                                    mediaBucketsList[position].id!!,
                                    selectedMediaPaths
                                )
                            }
    ...
    private fun onGalleryMediaClick(position: Int) {
        ...
        if (mediaList[position].selected) {
            ...
            if (settings.maxSelectableMediaCount != null && selectedMediaList.size == settings.maxSelectableMediaCount) {
                ...
                toastMaxMediaCountError =
                    this.toastStringRes(R.string.uwmediapicker_toast_error_max_media_selected)
            ...
            mediaList[position].mediaPath?.let {
                val mediaType =
                    if (mediaList[position] is GalleryImageModel) UwMediaPickerMediaType.IMAGE else UwMediaPickerMediaType.VIDEO
    ...
    private fun getProgressDialog(): AlertDialog {
        ...
        val dialogView =
            LayoutInflater.from(this).inflate(R.layout.uwmediapicker_dialog_progress, null)
                        ...
                        if (selectedMedia.mediaType == UwMediaPickerMediaType.IMAGE) {
                            try {
                                val compressedImageFile =
                                    imageCompressor.compress(File(selectedMedia.mediaPath))
                                selectedMediaList[index].mediaPath =
