...
internal class GalleryMediaDataProvider(private val context: Context) {
    ...
    private fun getSelectionImageByBucketId(bucketId: String) =
        "${MediaStore.MediaColumns.BUCKET_ID}=$bucketId"

    private fun getSelectionVideoByBucketId(bucketId: String) =
        "${MediaStore.MediaColumns.BUCKET_ID}=$bucketId"

    private fun getSelectionImageAndVideoByBucketId(bucketId: String) =
        "${getSelectionImageAndVideo()} AND ${MediaStore.MediaColumns.BUCKET_ID}=$bucketId"
    ...
    private fun getBuckets(
        cursor: Cursor?,
        queryUri: Uri,
        getSelectionByBucketIdFunction: (String) -> String
    ): ArrayList<GalleryMediaBucketModel> {
        ...
        if (cursor?.moveToLast() == true) {
            do {
                ...
                val bucketId =
                    cursor.getString(cursor.getColumnIndex(MediaStore.MediaColumns.BUCKET_ID))
                ...
                val bucketMediaCount =
                    getMediaCount(queryUri, getSelectionByBucketIdFunction(bucketId))
                bucketsList.add(
                    GalleryMediaBucketModel(
                        bucketId,
                        bucketName,
                        bucketImagePath,
                        bucketMediaCount
    ...
    fun getImagesOfBucket(
        bucketId: String,
        selectedMediaPathList: List<String>
    ): ArrayList<GalleryImageModel> {
        val cursor = context.contentResolver.query(
            MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
     ...
     * @param bucketId BucketId
     * @param selectedMediaPathList, for look to whether media already selected and mark them selected again */
    fun getVideosOfBucket(
        bucketId: String,
        selectedMediaPathList: List<String>
    ): ArrayList<GalleryVideoModel> {
        val cursor = context.contentResolver.query(
            MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
            arrayOf(
                MediaStore.MediaColumns.DATA,
    ...
    fun getImagesAndVideosOfBucket(
        bucketId: String,
        selectedMediaPathList: List<String>
    ): ArrayList<BaseGalleryMediaModel> {
        val queryUri = MediaStore.Files.getContentUri("external")
        val selection =
            "(${MediaStore.Files.FileColumns.MEDIA_TYPE}=${MediaStore.Files.FileColumns.MEDIA_TYPE_IMAGE}" +
                    " OR ${MediaStore.Files.FileColumns.MEDIA_TYPE}=${MediaStore.Files.FileColumns.MEDIA_TYPE_VIDEO})" +
