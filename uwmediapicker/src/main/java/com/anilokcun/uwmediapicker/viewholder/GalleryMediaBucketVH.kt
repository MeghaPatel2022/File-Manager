...
internal class GalleryMediaBucketVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    private val imgThumbnail =
        itemView.findViewById<ImageView>(R.id.item_gallery_media_bucket_img_thumbnail)
    private val tvName = itemView.findViewById<TextView>(R.id.item_gallery_media_bucket_tv_name)
    private val tvMediaCount =
        itemView.findViewById<TextView>(R.id.item_gallery_media_bucket_tv_media_count)
        ...
        Glide.with(itemView).apply {
            this.load(item.coverImagePath?.toUri())
                .apply(
                    RequestOptions
                        .overrideOf(
                            UwMediaPickerActivity.GALLERY_GRID_SIZE,
                            UwMediaPickerActivity.GALLERY_GRID_SIZE
                        )
                        .centerCrop()
                        .placeholder(
                            ColorDrawable(
                                ContextCompat.getColor(
                                    itemView.context,
                                    R.color.colorUwMediaPickerImagePlaceHolder
                                )
                            )
                        )
                )
