...
internal class GalleryVideoVH(itemView: View) : BaseGalleryMediaVH(itemView) {
    ...
    private val imgThumbnail =
        itemView.findViewById<ImageView>(R.id.item_gallery_video_img_thumbnail)
    private val imgSelected = itemView.findViewById<ImageView>(R.id.item_gallery_video_img_selected)
    private val tvVideoDuration =
        itemView.findViewById<TextView>(R.id.item_gallery_video_tv_video_duration)
    ...
    fun bind(item: GalleryVideoModel, onMediaClickListener: OnRVItemClickListener) {
        ...
        tvVideoDuration.text = if (videoDurationHour > 0) {
            itemView.context
                .getString(
                    R.string.uwmediapicker_time_format_hour_min_sec,
                    videoDurationHour, videoDurationMinute, videoDurationSecond
                )
        } else {
            itemView.context
                .getString(
                    R.string.uwmediapicker_time_format_min_sec,
            ...
            if (item.videoSize == "0") {
                ...
                TextUtils.concat(
                    DecimalFormat("#,##0.#").format(
                        item.videoSize.toDouble()
                                / (1024.0.pow(digitGroups.toDouble()))
                    ), " ", units[digitGroups]
                )
            }
        } else ""
