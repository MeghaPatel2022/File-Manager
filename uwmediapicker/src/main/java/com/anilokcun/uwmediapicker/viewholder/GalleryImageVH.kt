...
internal class GalleryImageVH(itemView: View) : BaseGalleryMediaVH(itemView) {
    ...
    private val imgThumbnail =
        itemView.findViewById<ImageView>(R.id.item_gallery_image_img_thumbnail)
    private val imgSelected = itemView.findViewById<ImageView>(R.id.item_gallery_image_img_selected)

    fun bind(
        item: GalleryImageModel,
        onMediaClickListener: OnRVItemClickListener,
        galleryMediaOnLongClickListener: GalleryMediaOnLongClickListener?
    ) {
