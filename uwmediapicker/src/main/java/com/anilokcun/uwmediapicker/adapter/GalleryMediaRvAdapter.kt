...
internal class GalleryMediaRvAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            BaseGalleryModel.TYPE_GALLERY_MEDIA_BUCKET -> GalleryMediaBucketVH(
                LayoutInflater.from(context)
                    .inflate(R.layout.uwmediapicker_item_gallery_media_bucket, parent, false)
            )
            // Gallery Video;
            BaseGalleryModel.TYPE_GALLERY_VIDEO -> GalleryVideoVH(
                LayoutInflater.from(context)
                    .inflate(R.layout.uwmediapicker_item_gallery_video, parent, false)
            )
            // Gallery Image;
            BaseGalleryModel.TYPE_GALLERY_IMAGE -> GalleryImageVH(
                LayoutInflater.from(context)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (itemList[position].itemType) {
            // Gallery Media Bucket;
            BaseGalleryModel.TYPE_GALLERY_MEDIA_BUCKET -> (holder as GalleryMediaBucketVH).bind(
                itemList[position] as GalleryMediaBucketModel,
                onRVItemClickListener
            )
            // Gallery Video;
            BaseGalleryModel.TYPE_GALLERY_VIDEO -> (holder as GalleryVideoVH).bind(
                itemList[position] as GalleryVideoModel,
                onRVItemClickListener
            )
            // Gallery Image;
            BaseGalleryModel.TYPE_GALLERY_IMAGE -> (holder as GalleryImageVH).bind(
                itemList[position] as GalleryImageModel,
                onRVItemClickListener,
                galleryMediaOnLongClickListener
            )
            // Error;
